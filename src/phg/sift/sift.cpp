...
void phg::SIFT::buildPyramids(const cv::Mat &imgOrg, std::vector<cv::Mat> &gaussianPyramid, std::vector<cv::Mat> &DoGPyramid) {
  ...
  for (size_t octave = 0; octave < NOCTAVES; ++octave) {
    if (octave == 0) {
      ...
      cv::Size new_size = img.size() / 2;

      cv::resize(img, img, new_size, 0.5, 0.5, cv::INTER_NEAREST);
...
#pragma omp parallel for
    for (ptrdiff_t layer = 1; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
      ...
      double sigma = sqrt(sigmaCur*sigmaCur - INITIAL_IMG_SIGMA * INITIAL_IMG_SIGMA);                // sigma2  - сигма которую надо добавить чтобы довести sigma1 до sigma12
...
#pragma omp parallel for
  ...
  // нам нужны padding-картинки по краям октавы чтобы извлекать экстремумы, но в статье предлагается не s+2 а s+3: [lowe04]
  // We must produce s + 3 images in the stack of blurred images for each octave, so that final extrema detection covers a complete octave
...
namespace {
float parabolaFitting(float x0, float x1, float x2, float& shift) {
  ...
  shift = - b / (2.0f * a) - 1.0f;
  return a * shift * shift + b * shift + x0;
}
}
...
#pragma omp for
        for (ptrdiff_t j = 1; j < cur.rows - 1; ++j) {
          for (ptrdiff_t i = 1; i + 1 < cur.cols; ++i) {
            ...
            {
              float df_dx = parabolaFitting(DoGs[1].at<float>(j, i - 1), center, DoGs[1].at<float>(j, i + 1), dx);
              float df_dy = parabolaFitting(DoGs[1].at<float>(j - 1, i), center, DoGs[1].at<float>(j + 1, i), dy);
              dvalue = df_dx + df_dy;
            };
#endif
            ...
            for (size_t bin = 0; bin < ORIENTATION_NHISTS; ++bin) {
              ...
              if (value > prevValue && value > nextValue && votes[bin] > biggestVote * ORIENTATION_VOTES_PEAK_RATIO) {
                float bin_shift = 0.0;
                parabolaFitting(prevValue, value, nextValue, bin_shift);
                kp.angle = (bin + bin_shift + 0.5) * (360.0 / ORIENTATION_NHISTS);
...
bool phg::SIFT::buildLocalOrientationHists(const cv::Mat &img, size_t i, size_t j, size_t radius,
  ...
  for (size_t y = j - radius + 1; y < j + radius; ++y) {
    for (size_t x = i - radius + 1; x < i + radius; ++x) {
      ...
      size_t bin = std::floor(orientation * ORIENTATION_NHISTS / 360);
        ...
        for (int smpi = 0; smpi < DESCRIPTOR_SAMPLES_N; ++smpi) { // перебираем столбик очередного замера для текущей гистограммы
          for (int smpy = 0; smpy < smpW; ++smpy) { // перебираем ряд пикселей текущего замера
            for (int smpx = 0; smpx < smpW; ++smpx) { // перебираем столбик пикселей текущего замера
              ...
              size_t bin = std::floor(orientation * DESCRIPTOR_NBINS / 360);
