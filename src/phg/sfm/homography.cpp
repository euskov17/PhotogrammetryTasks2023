...
//    throw std::runtime_error("not implemented yet");
    double point[3][1] = {{pt.x}, {pt.y}, {1.0}};
    cv::Mat transformed_point = T * cv::Mat(3, 1, CV_64F, point);
    double last_cord = transformed_point.at<double>(2,0);

    return cv::Point2d(transformed_point.at<double>(0, 0) / last_cord,
                       transformed_point.at<double>(1, 0) / last_cord);
}
