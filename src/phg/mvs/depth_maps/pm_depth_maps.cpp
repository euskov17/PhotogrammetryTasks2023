...
namespace phg {
        ...
        for (ptrdiff_t j = 0; j < height; ++j) {
            for (ptrdiff_t i = 0; i < width; ++i) {
...

                {
                    ...
                    float d = (1.0 - (float)iter / (float) NITERATIONS) / 2.0;
                    dp = r.nextf(d0 * (1.0 - d), d0 * (1.0 + d)); // TODO 104: сделайте так чтобы отклонение было тем меньше, чем номер итерации ближе к NITERATIONS, улучшило ли это результат?
                    np = cv::normalize(n0 + randomNormalObservedFromCamera(cameras_RtoWorld[ref_cam], r) * d); // TODO 105: сделайте так чтобы отклонение было тем меньше, чем номер итерации ближе к NITERATIONS, улучшило ли это результат?
    ...
    float PMDepthMapsBuilder::estimateCost(ptrdiff_t i, ptrdiff_t j, double d, const vector3d &global_normal, size_t neighb_cam)
    {
        ...
        for (size_t k = 0; k < n; ++k) {
            ...
            sigma0 += (patch0[k] - mean0) * (patch0[k] - mean0);
            sigma1 += (patch1[k] - mean1) * (patch1[k] - mean1);
        }

        zncc /= std::sqrt(1e-5 + sigma0 * sigma1);
    ...
    float PMDepthMapsBuilder::avgCost(std::vector<float> &costs)
    {
        ...
        int i = 0;
        while (i < costs.size() && i < COSTS_BEST_K_LIMIT && costs[i] <= COSTS_K_RATIO * best_cost) {
            cost_sum += costs[i++];
            cost_w++;
        }
        ...
        // Наверное можно добавить проверку, что cost <= Some_constant, но не особо понятно, как ее подбирать
